...
def metric(cases, method="new", ablate=None):
    ...
    return dict(n=n, top1=top[1]/n, top3=top[3]/n, top5=top[5]/n, top10=top[10]/n,
                ...
                mean_top3_regions=sum(diversity)/n, runtime_seconds=round(time.perf_counter()-t0,6), records=records)
...
def main():
    ...
    result["runtime_seconds_observed"]=elapsed
    ...
    try:
        ...
        fig.savefig(OUT_PNG, dpi=180, metadata={"Software":"OligoForge", "Creation Time":None})
        fig.savefig(OUT_SVG, metadata={"Creator":"OligoForge", "Date":None})
        plt.close(fig)
    except Exception as exc:
        result["figure_warning"]=type(exc).__name__
        OUT_JSON.write_text(json.dumps(result, indent=2)+"\n")
    files = [CORPUS, OUT_JSON, OUT_CSV, OUT_PNG, OUT_SVG]
    trace = ROOT / "benchmark" / "plasmodium_ranking_trace.json"
