...
def scan(base, markers, exclude=None, k_seqs=6, max_markers=6):
    ...
    for m in markers[:max_markers]:
        ...
        if exclude:
            try:
                off_count = ncbi.count_hits(("%s %s" % (exclude, m["qwords"])).strip())
            except Exception:
                off_count = None
        try:
        ...
        out.append(dict(
            gene=m["gene"], query=q, count=count, off_count=off_count, n=len(seqs),
            median_len=(lens[len(lens) // 2] if lens else None),
            min_len=(lens[0] if lens else None),
            max_len=(lens[-1] if lens else None),
        ))
    return dict(base=base, exclude=exclude, results=out)
