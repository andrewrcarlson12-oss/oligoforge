...
# 4. A generic sequence-only design must not pretend an exon-junction constraint was supplied.
#    Explicit junction ranking is pinned separately by test_junction.py, where the boundary is part
#    of the input.  Requiring an unmarked generic design to hit coordinate 1707 was a legacy test
#    bug: it rewarded hidden biological knowledge rather than the declared objective.
import json
...
check("SDHA generic design records junction evaluation as not performed",
      bool(a) and not ((a.get("ranking_evidence") or {}).get("evaluations") or {}).get("junction", False),
      (a or {}).get("ranking_evidence"))
T.set_conditions(anneal_c=60)
